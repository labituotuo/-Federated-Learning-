...
from multithread_federated.model import SimpleNN
...
def load_test_data(data_dir='clients_data'):
    """加载测试数据并应用标准化"""
    # 首先创建全局scaler（与训练时一致）
...
def generate_shap_bar_plot(model, X, feature_names, output_path):
    ...
    plt.title('多线程联邦学习模型 - SHAP特征重要性分析（前20位）', fontsize=18, fontweight='bold', pad=20)
...
if __name__ == '__main__':
    ...
    # 模型路径（指定的模型）
    model_path = 'multithread_fl_model_20260507_121204.pth'
    output_path = 'multithread_fl_model_20260507_121204_shap.svg'
    
    print("=" * 60)
    print("Multithread Federated Model SHAP Feature Analysis (Top 20)")
    print("=" * 60)
