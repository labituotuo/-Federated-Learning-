...
class HomomorphicCrypto:
    ...
    def generate_keypair(self):
        print(f'[加密] 生成 {self.key_size} 位 Paillier 密钥对...')
    ...
    def load_public_key_from_dict(self, pub_key_dict):
        self.public_key = paillier.PaillierPublicKey(n=pub_key_dict['n'])
    ...
    def encrypt_tensor(self, tensor):
        ...
        for val in arr:
            float_val = float(val)
            encrypted_values.append(self.public_key.encrypt(float_val))

        return encrypted_values

    def serialize_encrypted_tensor(self, encrypted_values):
        serialized = []
        for enc_val in encrypted_values:
            ct = enc_val._EncryptedNumber__ciphertext
    ...
    def deserialize_encrypted_tensor(self, serialized, original_shape=None):
        ...
        if original_shape is not None:
            return self._reshape_encrypted(encrypted_values, original_shape)
        return encrypted_values

    def _reshape_encrypted(self, flat_list, shape):
        result = []
        for i in range(shape[0]):
            if len(shape) == 1:
                ...
                for j in range(shape[1]):
                    idx = i * shape[1] + j
                    sublist.append(flat_list[idx])
                result.append(sublist)
        return result

    def decrypt_tensor(self, encrypted_values, original_shape=None):
        if self.private_key is None:
            raise ValueError('私钥未初始化')

        decrypted_values = [self.private_key.decrypt(e) for e in encrypted_values]
        result = np.array(decrypted_values)
    ...
    def aggregate_encrypted(self, encrypted_tensors_list, weights=None):
        ...
        first_tensor = encrypted_tensors_list[0]

        if isinstance(first_tensor[0], list):
            n_rows = len(first_tensor)
            aggregated = []
            for i in range(n_rows):
                row_tensors = [enc_tensor[i] for enc_tensor in encrypted_tensors_list]
                aggregated_row = self.aggregate_encrypted(row_tensors, weights)
                aggregated.append(aggregated_row)
            return aggregated
        else:
            n_values = len(first_tensor)
            aggregated = []
                ...
                for enc_tensor, weight in zip(encrypted_tensors_list, weights):
                    if weight == 1.0:
                        if total is None:
                            ...
                            total = total + enc_tensor[i]
                    else:
                        if total is None:
                            total = enc_tensor[i] * weight
                        else:
                            total = total + (enc_tensor[i] * weight)
    ...
    def save_keys(self, public_key_path, private_key_path):
        with open(public_key_path, 'wb') as f:
            pickle.dump(self.get_public_key_dict(), f)
    ...
    def load_keys(self, public_key_path, private_key_path=None):
        ...
        self.load_public_key_from_dict(pub_key_dict)

        if private_key_path and os.path.exists(private_key_path):
            with open(private_key_path, 'rb') as f:
                private_key_dict = pickle.load(f)
